...
from funciones_db import get_conn, init_db
    ...
    if menu == "📊 Dashboard":
        ...
        with get_conn() as conn:
            ventas = pd.read_sql_query(
                "SELECT v.id, v.fecha, v.cliente, v.detalle, v.monto_total FROM ventas v WHERE v.activo=1 ORDER BY v.fecha DESC LIMIT 100",
                conn,
            )
            gastos = pd.read_sql_query(
                "SELECT id, fecha, descripcion, monto FROM gastos WHERE activo=1 ORDER BY fecha DESC LIMIT 100",
                conn,
            )
            inventario = pd.read_sql_query(
                "SELECT id, item, variante, categoria, cantidad, minimo, unidad FROM inventario WHERE activo=1 ORDER BY item",
                conn,
            )
        ...
        with get_conn() as conn:
            inv = pd.read_sql_query(
                "SELECT id, item, variante, categoria, unidad, cantidad, minimo, precio_usd FROM inventario WHERE activo=1 ORDER BY item",
                conn,
            )
        ...
        with tab1:
            ...
            tipo = col2.selectbox(
                "Movimiento", ["ENTRADA", "SALIDA", "MERMA", "COMPRA", "ENVIO"]
            )
            ...
            if st.button("Registrar movimiento"):
                try:
                    if tipo == "MERMA":
                        registrar_merma(
                            int(item_id), float(cantidad), usuario=usuario, motivo=motivo
                        )
                    elif tipo == "COMPRA":
                        registrar_compra(
                            int(item_id), float(cantidad), usuario=usuario, motivo=motivo
                        ...
                        enviar_inventario(
                            int(item_id), float(cantidad), destino=motivo, usuario=usuario
                        )
                    else:
                        registrar_movimiento_inventario(
                            int(item_id),
        ...
        with tab2:
            ...
            if st.button("Crear variante"):
                with get_conn() as conn:
                    conn.execute(
                        "INSERT INTO inventario(item, variante, categoria, unidad, cantidad, minimo, precio_usd, activo) VALUES (?,?,?,?,?,?,?,1)",
                        (item, variante, categoria, "unidad", 0, 0, 0),
                    )
        ...
        tipo_papel = st.selectbox(
            "Tipo de papel del driver", ["fotográfico", "bond", "adhesivo"]
        )
        with get_conn() as conn:
            impresoras = conn.execute(
                "SELECT id, equipo, modelo FROM activos WHERE activo=1 AND tipo='impresora'"
            ).fetchall()
            ids = [
                row["id"]
                for row in conn.execute(
                    "SELECT id FROM inventario WHERE activo=1 AND imprimible_cmyk=1"
                ).fetchall()
            ]
        ...
        if st.button("Analizar consumo CMYK"):
            resultado = analizar_cmyk(
                ids,
                calidad=calidad,
                tipo_papel=tipo_papel,
                impresora_id=op_imp.get(imp_sel) if imp_sel else None,
            )
        ...
        hoja_file = st.file_uploader(
            "Subir hoja de diagnóstico (PDF/JPG/TXT)",
            type=["pdf", "jpg", "jpeg", "png", "txt"],
        )
        tanques_file = st.file_uploader(
            "Subir foto tanques (JPG/PNG/TXT)",
            type=["jpg", "jpeg", "png", "txt"],
        )
        hoja = hoja_file.getvalue().decode("utf-8", errors="ignore") if hoja_file else ""
        foto = (
            tanques_file.getvalue().decode("utf-8", errors="ignore")
            if tanques_file
            else ""
        )
        ...
        for canal in ["C", "M", "Y", "K"]:
            selected = (
                st.selectbox(
                    f"Insumo inventario para tinta {canal}",
                    list(opciones.keys()),
                    key=f"tinta_{canal}",
                )
                if opciones
        ...
        if st.button("Procesar diagnóstico IA"):
            try:
                ...
                niveles = registrar_diagnostico_impresora(
                    int(activo["id"]), hoja, foto, map_ids, usuario=usuario
                )
        ...
        with get_conn() as conn:
            productos = [
                ...
                for r in conn.execute(
                    "SELECT DISTINCT producto FROM recetas_produccion WHERE activo=1 AND proceso='Sublimación'"
                )
            ]
        ...
        with get_conn() as conn:
            productos = [
                row["producto"]
                for row in conn.execute(
                    "SELECT DISTINCT producto FROM recetas_produccion WHERE activo=1 AND proceso='Corte Cameo'"
                )
            ]
        ...
        if st.button("Ejecutar proceso personalizado"):
            try:
                orden_id = ejecutar_produccion(
                    proceso, producto, cantidad, usuario=usuario, minutos_reales=minutos
                )
        ...
        if st.button("Aplicar ajuste"):
            try:
                aplicar_ajuste(
                    int(item_id), float(nuevo_stock), motivo=motivo, usuario=usuario
                )
        ...
        if alertas:
            st.dataframe(pd.DataFrame([dict(row) for row in alertas]), use_container_width=True)
        else:
