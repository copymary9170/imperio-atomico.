...
def registrar_diagnostico_impresora(activo_id, hoja_texto, foto_texto, mapa_tinta_item_id, usuario='Sistema'):
    ...
    with get_conn() as conn:
        conn.execute(
            '''
            INSERT INTO diagnosticos_impresora(activo_id, hoja_diagnostico, foto_tanques, niveles_json, observacion)
            VALUES (?, ?, ?, ?, ?)
            ''',
            ...
            if canal in mapa_tinta_item_id:
                ...
                if consumo_estimado > 0:
                    registrar_movimiento_inventario(
                        item_id=mapa_tinta_item_id[canal],
                        tipo='SALIDA',
