...
PREFERENCIAS_CONTACTO = ["WhatsApp", "Llamada", "Instagram", "Facebook", "Presencial", "No contactar"]
...
def _add_missing_columns(conn: Any, table_name: str, definitions: dict[str, str]) -> None:
    cols = _columns(conn, table_name)
    for campo, definicion in definitions.items():
        if campo not in cols:
            conn.execute(f"ALTER TABLE {table_name} ADD COLUMN {campo} {definicion}")
            cols.add(campo)
...
def cargar_clientes_mejorados() -> pd.DataFrame:
    ...
    with db_transaction() as conn:
        ...
        if _table_exists(conn, "cuentas_por_cobrar") and {"cliente_id", "saldo_usd", "estado"}.issubset(_columns(conn, "cuentas_por_cobrar")):
            ...
            """
            cxc_field = "COALESCE(pxc.deuda_total,0) AS deuda_usd"
        return pd.read_sql_query(f"""
            SELECT c.id, COALESCE(c.fecha,'') AS fecha, c.nombre,
                   COALESCE(c.telefono,'') AS whatsapp, COALESCE(c.email,'') AS email,
                   COALESCE(c.direccion,'') AS direccion, COALESCE(c.categoria,'General') AS categoria,
                   COALESCE(c.tipo_cliente,'General') AS tipo_cliente, COALESCE(c.origen,'No definido') AS origen,
...
def actualizar_datos_comerciales_cliente(cliente_id: int, *, categoria: str, tipo_cliente: str, origen: str, servicio_interes: str, comportamiento_pago: str, preferencia_contacto: str, acepta_promociones: bool, cumpleanos: str, fecha_especial: str, observaciones_comerciales: str, descuento_pct: float, lista_precio: str) -> None:
    ensure_clientes_mejoras_schema()
    with db_transaction() as conn:
        conn.execute("""
            UPDATE clientes
...
def render_mejoras_clientes(usuario: str = "Sistema") -> None:
    ...
    with tab_reportes:
        ...
        for campo, titulo in [("origen", "Clientes por origen"), ("tipo_cliente", "Clientes por tipo"), ("servicio_interes", "Clientes por servicio principal"), ("comportamiento_pago", "Clientes por comportamiento de pago")]:
    ...
    with tab_reactivar:
        ...
        reactivar = work[(work["operaciones"] > 0) & (work["dias_sin_compra"] >= int(rango)) & (work["acepta_promociones"].fillna(0).astype(int).eq(1))].copy()
    ...
    with tab_calidad:
        ...
        q1, q2, q3, q4 = st.columns(4); q1.metric("Teléfonos duplicados", len(duplicados_tel)); q2.metric("Sin teléfono", len(sin_tel)); q3.metric("Sin origen", len(sin_origen)); q4.metric("Sin servicio", len(sin_servicio))
        st.markdown("#### Teléfonos duplicados"); st.dataframe(duplicados_tel, use_container_width=True, hide_index=True)
        st.markdown("#### Clientes sin datos comerciales completos"); st.dataframe(pd.concat([sin_tel, sin_origen, sin_servicio]).drop_duplicates(subset=["id"]), use_container_width=True, hide_index=True)
