...
def create_producto(usuario: str, sku: str, nombre: str, categoria: str, unidad: str, costo: float, precio: float) -> int:
    ...
    with db_transaction() as conn:
        cur = conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (usuario, sku, nombre, categoria, unidad, money(costo), money(precio)),
        )
...
def add_inventory_movement(usuario: str, inventario_id: int, tipo: str, cantidad: float, costo_unitario_usd: float, referencia: str) -> None:
    ...
    with db_transaction() as conn:
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (usuario, inventario_id, tipo, cantidad, money(costo_unitario_usd), referencia),
        )
        conn.execute(
...
def render_inventario(usuario: str) -> None:
    ...
    with db_transaction() as conn:
        ...
        rows = conn.execute(
            "SELECT id, sku, nombre, categoria, stock_actual, costo_unitario_usd, precio_venta_usd FROM inventario WHERE estado='activo' ORDER BY id DESC"
        ).fetchall()

    c1, c2, c3 = st.columns(3)
    c1.metric("Productos activos", int(totals["total"] or 0))
    c2.metric("Stock total", f"{float(totals['stock'] or 0):,.2f}")
    c3.metric("Valor inventario", f"$ {float(totals['valor_stock'] or 0):,.2f}")
