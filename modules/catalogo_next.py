...
def _current_user() -> str:
    return _safe_text(st.session_state.get("usuario"), "Sistema") or "Sistema"
...
def _insert_catalog_item(conn: Any, usuario: str, data: dict[str, Any]) -> int:
    cur = conn.execute(
        ...
        INSERT INTO catalogo_items(
            ...
            imagen_url, ultima_revision_precio, activo
        ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)
        """,
    ...
    with db_transaction() as conn:
        df = pd.read_sql_query(
            ...
            SELECT
                ...
                stock_objetivo, orden_minima, lead_time_comercial_dias,
                visible_catalogo_publico, prioridad_comercial, temporada, coleccion,
                imagen_url, ultima_revision_precio
            FROM catalogo_items
...
def _filter_catalog(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if txt:
        fields = [
            "sku", "nombre", "categoria", "subcategoria", "tipo", "descripcion",
            "tags", "proveedor_sugerido", "ruta_base", "notas_tecnicas", "coleccion", "temporada",
        ]
        mask = False
        for field in fields:
...
def _render_metrics(df: pd.DataFrame) -> None:
    ...
    c2.metric("Publicados", int(df["visible_catalogo_publico"].fillna(0).astype(int).sum()))
...
def _collect_form_values(prefix: str, base: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    categoria = c4.selectbox("Categoría", DEFAULT_CATEGORIES, index=DEFAULT_CATEGORIES.index(base.get("categoria")) if base.get("categoria") in DEFAULT_CATEGORIES else 0, key=f"{prefix}_categoria")
    subcategoria = c5.text_input("Subcategoría", value=_safe_text(base.get("subcategoria")), key=f"{prefix}_subcategoria")
    tipo = c6.selectbox("Tipo", DEFAULT_TYPES, index=DEFAULT_TYPES.index(base.get("tipo")) if base.get("tipo") in DEFAULT_TYPES else 0, key=f"{prefix}_tipo")
    unidad = c7.selectbox("Unidad", DEFAULT_UNITS, index=DEFAULT_UNITS.index(base.get("unidad")) if base.get("unidad") in DEFAULT_UNITS else 0, key=f"{prefix}_unidad")
    ...
    canal = c8.selectbox("Canal principal", DEFAULT_CHANNELS, index=DEFAULT_CHANNELS.index(base.get("canal")) if base.get("canal") in DEFAULT_CHANNELS else 0, key=f"{prefix}_canal")
    tiempo_entrega = c9.number_input("Entrega producción días", min_value=0, value=_safe_int(base.get("tiempo_entrega_dias")), step=1, key=f"{prefix}_tiempo")
    lead_time = c10.number_input("Promesa comercial días", min_value=0, value=_safe_int(base.get("lead_time_comercial_dias"), _safe_int(base.get("tiempo_entrega_dias"))), step=1, key=f"{prefix}_lead")
    prioridad = c11.slider("Prioridad comercial", min_value=0, max_value=100, value=_safe_int(base.get("prioridad_comercial"), 50), key=f"{prefix}_prioridad")
    ...
    orden_minima = c13.number_input("Orden mínima", min_value=1, value=max(1, _safe_int(base.get("orden_minima"), 1)), step=1, key=f"{prefix}_orden_minima")
    ...
    costo_base = c15.number_input("Costo base ref.", min_value=0.0, value=_safe_float(base.get("costo_base_referencial"), _safe_float(base.get("costo"))), step=1.0, key=f"{prefix}_costo_base")
    ...
    imagen_url = st.text_input("URL imagen", value=_safe_text(base.get("imagen_url")), key=f"{prefix}_imagen")
...
def _render_export_import(df: pd.DataFrame, usuario: str) -> None:
    ...
    if _can_export() and not df.empty:
        export_cols = [
            "sku", "nombre", "categoria", "subcategoria", "tipo", "descripcion", "unidad",
            "precio", "costo", "precio_mayorista", "precio_minimo", "margen_pct",
            "tiempo_entrega_dias", "lead_time_comercial_dias", "canal", "estado",
            "proveedor_sugerido", "tags", "destacado", "visible_catalogo_publico",
            "prioridad_comercial", "score_comercial", "temporada", "coleccion",
            "ruta_base", "notas_tecnicas",
        ]
        csv = df[export_cols].to_csv(index=False).encode("utf-8-sig")
        st.download_button("⬇️ Exportar catálogo CSV", data=csv, file_name="catalogo_imperio_atomico.csv", mime="text/csv", use_container_width=True)
    else:
...
def _render_insights(df: pd.DataFrame) -> None:
    ...
    c1, c2 = st.columns(2)
    with c1:
        por_categoria = df.groupby("categoria", as_index=False).agg(items=("id", "count"), ticket=("precio", "mean"), margen=("margen_pct", "mean"))
    ...
    with c2:
        top_score = df.sort_values("score_comercial", ascending=False).head(10)[["sku", "nombre", "score_comercial", "margen_pct", "precio"]]
        st.caption("Top productos por score comercial")
        st.dataframe(top_score, use_container_width=True, hide_index=True)

    alertas = df[(df["margen_pct"] < 25) | (df["precio"] <= df["costo"]) | (df["visible_catalogo_publico"] == 0)].copy()
    if not alertas.empty:
        st.warning("Productos que requieren revisión de precio, margen o visibilidad.")
        st.dataframe(alertas[["sku", "nombre", "precio", "costo", "margen_pct", "visible_catalogo_publico", "estado"]], use_container_width=True, hide_index=True)
...
def render_catalogo_hub(usuario: str) -> None:
    ...
    st.caption("Catálogo comercial avanzado: productos, servicios, precios, rutas, canales y readiness para ventas.")
    ...
    with tab_catalog:
        ...
        if view_mode == "Tarjetas":
            ...
            cols = [
                "sku", "nombre", "categoria", "tipo", "estado", "canal", "precio", "costo",
                "utilidad_usd", "margen_pct", "score_comercial", "prioridad_comercial",
                "visible_catalogo_publico", "ruta_base",
            ]
