...
def _finalizar_dia(usuario: str, fondo_final: float, observaciones: str, resumen: dict, caja_esperada: float, diferencia: float, estatus_cierre: str) -> None:
    ...
    with db_transaction() as conn:
        conn.execute(
            ...
            INSERT INTO cierres_caja (
                fecha, usuario, estado, inicio_efectivo, ventas_efectivo,
                ventas_transferencia, ventas_pago_movil, ventas_binance,
                gastos_efectivo, gastos_transferencia, fin_efectivo,
                observaciones, dia_operacion_id, ventas_totales_usd,
                gastos_totales_usd, ventas_zelle, caja_esperada_usd,
                caja_contada_usd, diferencia_usd, estatus_cierre
            ) VALUES (?, ?, 'cerrado', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                caracas_timestamp(), usuario, _safe_float(dia["fondo_inicial_usd"]),
                _safe_float(resumen.get("ingreso_efectivo")), _safe_float(resumen.get("ingreso_transferencia")),
                _safe_float(resumen.get("ingreso_pago_movil")), _safe_float(resumen.get("ingreso_binance")),
                _safe_float(resumen.get("egreso_efectivo")), 0.0, _safe_float(fondo_final),
                observaciones, dia["id"], _safe_float(resumen.get("ingresos")), _safe_float(resumen.get("egresos")),
                _safe_float(resumen.get("ingreso_zelle")), _safe_float(caja_esperada), _safe_float(fondo_final),
                _safe_float(diferencia), estatus_cierre,
            ),
        )
        conn.execute(
            """
            UPDATE dias_operacion
            SET estado='cerrado', fecha_fin=?, usuario_fin=?, fondo_final_usd=?, observaciones_fin=?,
                ventas_usd=?, gastos_usd=?, caja_esperada_usd=?, diferencia_usd=?, estatus_cierre=?
            WHERE id=?
            """,
            (
                caracas_timestamp(), usuario, fondo_final, observaciones,
                _safe_float(resumen.get("ingresos")), _safe_float(resumen.get("egresos")),
                _safe_float(caja_esperada), _safe_float(diferencia), estatus_cierre, dia["id"],
            ),
        )
    ...
    with tab_fin:
        ...
        if not dia:
            ...
            if st.button("Finalizar día y guardar cierre de caja", type="primary", use_container_width=True):
                _finalizar_dia(usuario, fondo_final, obs_fin, resumen, caja_esperada, diferencia_final, estatus_final)
