...
def _save_cierre(data: dict[str, Any]) -> int:
    ...
    with db_transaction() as conn:
        cur = conn.execute(
            ...
            """,
            (
                data["fecha_operativa"], data["turno"], data["cajero"], data["usuario"], data.get("hora_inicio"), data.get("hora_fin"),
                float(data["efectivo_inicial_usd"]), float(data["efectivo_ingresos_usd"]), float(data["efectivo_egresos_usd"]),
                float(data["efectivo_esperado_usd"]), float(data["efectivo_contado_usd"]), float(data["diferencia_efectivo_usd"]),
                float(data["transferencia_usd"]), float(data["zelle_usd"]), float(data["binance_usd"]), float(data["total_sistema_usd"]),
                float(data["total_declarado_usd"]), float(data["diferencia_total_usd"]), data["estado"], data.get("observaciones"),
            ),
        )
...
def render_cierre_caja_turnos(usuario: str = "Sistema") -> None:
    ...
    if not puede_cerrar:
        st.info("Modo consulta: puedes ver historial y alertas, pero no registrar cierres de turno.")
    _ensure_tables()
    ...
    with tab_cerrar:
        ...
        st.info(f"Periodo calculado: {fecha_desde} → {fecha_hasta}. Ingresos efectivo: ${resumen['ing_efectivo']:,.2f} · Egresos efectivo: ${resumen['egr_efectivo']:,.2f}")
        ...
        with st.form("form_cierre_caja_turno"):
            ...
            transferencia_decl = p1.number_input("Transferencia declarada USD", min_value=0.0, value=float(resumen["transferencia"]), step=0.01, disabled=not puede_cerrar)
            zelle_decl = p2.number_input("Zelle declarado USD", min_value=0.0, value=float(resumen["zelle"]), step=0.01, disabled=not puede_cerrar)
            binance_decl = p3.number_input("Binance declarado USD", min_value=0.0, value=float(resumen["binance"]), step=0.01, disabled=not puede_cerrar)
    ...
    with tab_historial:
        if cierres.empty:
            ...
            resumen_cajero = cierres.groupby("cajero", as_index=False).agg(cierres=("id", "count"), diferencia_efectivo=("diferencia_efectivo_usd", "sum"), diferencia_total=("diferencia_total_usd", "sum"))
