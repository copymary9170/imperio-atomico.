...
ALIASES = {
    ...
    "tipo": "tipo_uso", "tipo de uso": "tipo_uso", "uso": "tipo_uso",
    "unidad": "unidad_base", "unidad base": "unidad_base", "unidad_base": "unidad_base",
    "fraccionable": "permite_fraccionamiento", "permite fraccionamiento": "permite_fraccionamiento",
    "marca": "marca", "color": "color",
    "tamano": "tamano", "tamaño": "tamano", "medida": "tamano", "nombre comercial": "tamano",
    "gramaje": "gramaje", "grosor": "gramaje", "acabado": "acabado",
    "ancho": "ancho_cm", "ancho cm": "ancho_cm",
    "alto": "alto_cm", "alto cm": "alto_cm",
    "merma": "merma_base_pct", "merma base": "merma_base_pct", "merma base adicional": "merma_base_pct",
    "margen izquierdo": "margen_izquierdo_cm", "margen derecho": "margen_derecho_cm",
    "margen superior": "margen_superior_cm", "margen inferior": "margen_inferior_cm",
    "separacion": "separacion_cm", "separación": "separacion_cm", "sangrado": "sangrado_cm",
    "unidad compra": "unidad_compra", "unidad de compra": "unidad_compra",
    "contenido": "contenido_compra", "contenido compra": "contenido_compra", "contenido por unidad de compra": "contenido_compra",
    "proveedor": "proveedor_principal", "proveedor principal": "proveedor_principal",
    "ubicacion": "ubicacion", "ubicación": "ubicacion",
    "stock": "stock_actual", "stock inicial": "stock_actual",
    "stock mínimo": "stock_minimo", "stock minimo": "stock_minimo", "mínimo": "stock_minimo", "minimo": "stock_minimo",
    "punto reorden": "punto_reorden", "punto de reorden": "punto_reorden",
    "stock ideal": "stock_ideal", "stock máximo": "stock_maximo", "stock maximo": "stock_maximo",
    "costo": "costo_unitario_usd", "costo unitario": "costo_unitario_usd", "costo unitario usd": "costo_unitario_usd",
    "precio": "precio_venta_usd", "precio venta": "precio_venta_usd", "precio venta usd": "precio_venta_usd",
    "observacion": "observaciones", "observación": "observaciones", "observaciones": "observaciones",
}
...
DEFAULT_ITEM = {
    "sku": "", "nombre": "", "categoria": "General", "tipo_uso": "Ambos", "unidad_base": "unidad",
    "permite_fraccionamiento": True, "marca": "", "color": "", "tamano": "", "gramaje": "", "acabado": "",
    "ancho_cm": 0.0, "alto_cm": 0.0, "margen_izquierdo_cm": 0.0, "margen_derecho_cm": 0.0,
    "margen_superior_cm": 0.0, "margen_inferior_cm": 0.0, "separacion_cm": 0.0, "sangrado_cm": 0.0,
    "merma_base_pct": 0.0, "unidad_compra": "", "contenido_compra": 0.0, "proveedor_principal": "", "ubicacion": "",
    "stock_actual": 0.0, "stock_minimo": 0.0, "punto_reorden": 0.0, "stock_ideal": 0.0, "stock_maximo": 0.0,
...
def _to_float(value: Any) -> float:
    if value is None:
        return 0.0
    text = str(value).strip().replace("$", "").replace("%", "").replace("cm", "").replace("g", "")
    text = text.replace(",", ".")
    try:
        return float(text)
    except Exception:
...
def _to_bool(value: Any) -> bool:
    return str(value or "").strip().lower() in {"si", "sí", "s", "true", "1", "x", "yes", "y"}
...
def _is_key_value_text(text: str) -> bool:
    ...
    for line in lines:
        if "=" in line:
            ...
            continue
        if _normalize_key(key) in DEFAULT_ITEM:
            count += 1
    return count >= 2
    ...
    if has_header:
        for row in rows[1:]:
            if any(str(cell).strip() for cell in row):
                items.append(_normalize_item({header[i]: row[i] if i < len(row) else "" for i in range(len(header))}))
    else:
        for row in rows:
            if any(str(cell).strip() for cell in row):
                items.append(_normalize_item({CSV_COLUMNS[i]: row[i] if i < len(row) else "" for i in range(min(len(CSV_COLUMNS), len(row)))}))
    return items
...
def _render_pegar_datos(usuario: str) -> None:
    ...
    if len(items) == 1:
        area_total, area_util, merma = _calcular_areas(items[0])
...
def _render_form_crear(usuario: str) -> None:
    ...
    with st.form("form_crear_item_unificado"):
        ...
        categoria = c3.selectbox("Categoría", CATEGORIAS_SUGERIDAS, index=CATEGORIAS_SUGERIDAS.index("General"))
...
def _render_editar(usuario: str) -> None:
    ...
    with st.form("editar_basico_inventario"):
        ...
        nombre = st.text_input("Nombre", value=str(row["nombre"] or ""))
        categoria = st.text_input("Categoría", value=str(row["categoria"] or "General"))
        c1, c2, c3 = st.columns(3)
        marca = c1.text_input("Marca", value=str(row["marca"] or ""))
        color = c2.text_input("Color", value=str(row["color"] or ""))
        tamano = c3.text_input("Tamaño", value=str(row["tamano"] or ""))
...
def _render_clasificar() -> None:
    ...
    with st.form("form_clasificar_item"):
        ...
        tipo_nuevo = st.selectbox("Tipo de uso", TIPOS_USO, index=TIPOS_USO.index(tipo_actual) if tipo_actual in TIPOS_USO else 2)
        unidad_nueva = st.selectbox("Unidad base", unidades, index=unidades.index(unidad_actual))
        fraccionable_nuevo = st.checkbox("Permite fraccionamiento", value=bool(int(row.get("permite_fraccionamiento") or 0)))
        guardar = st.form_submit_button("Guardar clasificación", type="primary")
    if guardar:
