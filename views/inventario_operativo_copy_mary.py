...
def _selector_articulo(label: str, key: str) -> int:
    df = listar_articulos()
    if df.empty:
        st.warning("No hay artículos activos en inventario.")
        st.stop()
    ids = [int(x) for x in df["id"].tolist()]
    mapa = {int(r["id"]): f"{r['nombre']} · {r['stock_disponible']:.2f} {r['unidad']} disponibles" for _, r in df.iterrows()}
...
def render_inventario_operativo_copy_mary(usuario: str) -> None:
    ...
    with tabs[1]:
        ...
        if ok:
            try:
                registrar_movimiento(inventario_id=item_id, tipo=tipo, cantidad=cantidad, costo_unitario=costo, motivo=motivo, usuario=usuario)
                st.success("Movimiento registrado y stock actualizado."); st.rerun()
    ...
    with tabs[2]:
        ...
        if ok:
            try:
                ...
                st.success(f"Reserva #{rid} creada."); st.rerun()
            except Exception as exc: st.error(str(exc))
        reservas = listar_reservas(); st.dataframe(reservas, use_container_width=True, hide_index=True)
        activas = reservas[reservas["estado"] == "activa"] if not reservas.empty else reservas
        if not activas.empty:
            rid = st.selectbox("Reserva activa", [int(x) for x in activas["id"]])
            c1, c2 = st.columns(2)
            if c1.button("Consumir reserva", use_container_width=True):
                try: consumir_reserva(rid, usuario); st.success("Reserva consumida y descontada."); st.rerun()
                except Exception as exc: st.error(str(exc))
            if c2.button("Liberar reserva", use_container_width=True):
                liberar_reserva(rid); st.success("Reserva liberada."); st.rerun()

    with tabs[3]:
        st.markdown("#### Crear receta o ficha de materiales")
        articulos = listar_articulos(); ids = [None] + [int(x) for x in articulos["id"].tolist()]
        nombres = {None: "Servicio sin producto terminado"}; nombres.update({int(r["id"]): str(r["nombre"]) for _, r in articulos.iterrows()})
        ...
        if ok:
            try: st.success(f"Receta #{crear_receta(nombre=nombre, producto_id=producto, rendimiento=rendimiento, unidad=unidad, observaciones=obs, usuario=usuario)} creada."); st.rerun()
            except Exception as exc: st.error(str(exc))
        recetas = listar_recetas(); st.dataframe(recetas, use_container_width=True, hide_index=True)
        if not recetas.empty:
            st.markdown("#### Agregar material a receta")
            with st.form("agregar_insumo"):
                receta_id = st.selectbox("Receta", [int(x) for x in recetas["id"]], format_func=lambda x: str(recetas.loc[recetas["id"] == x, "nombre"].iloc[0]))
            ...
            if ok:
                try: agregar_insumo_receta(receta_id=receta_id, insumo_id=insumo_id, cantidad=cantidad, merma_pct=merma); st.success("Material agregado."); st.rerun()
                except Exception as exc: st.error(str(exc))

    with tabs[4]:
        recetas = listar_recetas()
        if recetas.empty: st.info("Primero crea una receta y agrega sus materiales.")
        else:
            ...
            if ok:
                try: producir(receta_id=receta_id, cantidad_producir=cantidad, usuario=usuario, referencia=referencia); st.success("Producción procesada y materiales descontados."); st.rerun()
                except Exception as exc: st.error(str(exc))
    ...
    with tabs[5]:
        ...
        if ok:
            try: registrar_merma(inventario_id=item_id, cantidad=cantidad, motivo=motivo, referencia=referencia, usuario=usuario); st.success("Merma registrada y valorizada."); st.rerun()
            except Exception as exc: st.error(str(exc))
    ...
    with tabs[6]:
        ...
        if ok:
            try:
                ...
                st.success(f"Conteo registrado. Diferencia aplicada: {diferencia:+.2f}."); st.rerun()
            except Exception as exc: st.error(str(exc))
