...
from services.inventario_costeo_elite_service import (
    calcular_corte,
    guardar_costeo,
    listar_costeo_elite,
)
...
def render_inventario_costeo_elite(usuario: str) -> None:
    st.subheader("📐 Costeo técnico y unidades reales")
    st.caption(
        "Calcula el costo puesto en almacén incluyendo producto, delivery, impuestos, "
        "comisión, otros gastos, merma y unidad real de consumo."
    )

    df = listar_costeo_elite()
    if df.empty:
        st.info("Primero crea artículos en Operación y existencias → Maestro.")
        return

    tabs = st.tabs(["💰 Costo puesto", "✂️ Rendimiento por corte", "📊 Resumen técnico"])
