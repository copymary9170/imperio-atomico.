...
CALIDAD_FACTOR = {
    'borrador': 'factor_borrador',
    'normal': 'factor_normal',
    'alta': 'factor_alta',
}
PAPEL_FACTOR = {
    'fotográfico': 'factor_papel_fotografico',
    'bond': 'factor_papel_bond',
    'adhesivo': 'factor_papel_adhesivo',
}
...
def analizar_cmyk(item_ids, calidad='normal', tipo_papel='bond'):
    with get_conn() as conn:
        placeholders = ','.join(['?'] * len(item_ids)) if item_ids else 'NULL'
        rows = conn.execute(
            f'''
            SELECT id, item, cantidad, perfil_color
            FROM inventario
            WHERE activo=1 AND imprimible_cmyk=1 AND id IN ({placeholders})
            ''',
            tuple(item_ids),
        ...
        salida.append(
            {
                'item_id': r['id'],
                'item': r['item'],
                'stock': float(r['cantidad'] or 0),
                'consumo_estimado_ml': round(consumo, 4),
            }
        )
    return salida
...
def _crear_orden(proceso, producto, cantidad, usuario, activo_id=None):
    with get_conn() as conn:
        cur = conn.execute(
            '''
            INSERT INTO ordenes_produccion(proceso, producto, cantidad, usuario, activo_id)
            VALUES (?,?,?,?,?)
            ''',
...
def ejecutar_produccion(proceso, producto, cantidad, usuario='Sistema', minutos_reales=0, area=1):
    ...
    for insumo in receta:
        ...
        registrar_movimiento_inventario(
            ...
            tipo='SALIDA',
            cantidad=consumo,
            motivo=f'Consumo receta {proceso}',
            referencia=f'OP-{orden_id}',
            usuario=usuario,
        )

    if activo_id:
        registrar_uso_activo(activo_id, uso_incremental=cantidad, usuario=usuario, referencia=f'OP-{orden_id}')

    if minutos_reales and float(minutos_reales) > 0:
        _registrar_tiempo(orden_id, float(minutos_reales), usuario)
