...
class InventoryService:
    ...
    def procesar_movimiento(self, conn, movement: InventoryMovement) -> Tuple[bool, str]:
        ...
        try:
            ...
            if tipo_txt not in TipoMovimiento.__members__:
            ...
            motivo_txt = str(movement.motivo or "").strip()
        ...
        try:
            ...
            row = conn.execute(
                ...
                SELECT item,
                       COALESCE(cantidad,0),
                       COALESCE(costo_promedio,COALESCE(precio_usd,0)),
            ...
            if base_tipo == "ENTRADA":
                ...
                costo_prom_nuevo = (
                    valor_total_nuevo / saldo_despues if saldo_despues > 0 else 0.0
                )

                costo_unit_mov = costo_unitario

                costo_total_mov = valor_nuevo

            elif base_tipo == "SALIDA":

                if cantidad > saldo_antes:
                    raise ValueError("Stock insuficiente para salida")

                saldo_despues = saldo_antes - cantidad

                costo_unit_mov = costo_prom_actual

                costo_total_mov = cantidad * costo_unit_mov
                ...
                es_resta = cantidad < 0 or any(
                    k in motivo_txt.lower() for k in ["rest", "salida", "rebaja"]
                )
                ...
                if es_resta:
                    ...
                    valor_total_nuevo = max(
                        0.0, valor_total_actual - costo_total_mov
                    )
            ...
            self.registrar_kardex(
                ...
                stock_nuevo=saldo_despues,
                costo_unit=costo_unit_mov,
                usuario=usuario_txt,
            )
