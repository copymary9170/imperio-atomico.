from __future__ import annotations

from dataclasses import dataclass

from services.costo_real_compra_service import calcular_costo_real_compra


@dataclass(frozen=True)
class CostoCabezal:
    cantidad_cabezales: float
    costo_base_cabezales_usd: float
    impuesto_usd: float
    delivery_usd: float
    comision_pago_usd: float
    costo_total_cabezales_usd: float
    costo_unitario_real_cabezal_usd: float
    vida_util_paginas: float
    costo_cabezal_por_pagina_usd: float
    costo_cabezal_por_trabajo_usd: float


def calcular_costo_cabezales(
    *,
    cantidad_cabezales: float,
    costo_unitario_cabezal_usd: float,
    vida_util_paginas: float,
    paginas_trabajo: float = 1.0,
    impuestos_pct: float = 0.0,
    delivery_usd: float = 0.0,
    comision_pago_usd: float = 0.0,
) -> CostoCabezal:
    """Prorratea el costo real de cabezales por página o trabajo.

    Incluye precio de compra, impuestos, delivery/envío y comisión de pago.
    Útil para impresoras de tanque como HP Smart Tank, Epson EcoTank u otras
    máquinas donde el cabezal es una pieza de desgaste aparte de la tinta.
    """
    cantidad = max(0.0, float(cantidad_cabezales or 0.0))
    costo_unit = max(0.0, float(costo_unitario_cabezal_usd or 0.0))
    vida = max(0.0, float(vida_util_paginas or 0.0))
    paginas = max(0.0, float(paginas_trabajo or 0.0))

    costo_base_total = cantidad * costo_unit
    costo_real = calcular_costo_real_compra(
        costo_base_usd=costo_base_total,
        cantidad=max(cantidad, 1.0),
        impuestos_pct=impuestos_pct,
        delivery_usd=delivery_usd,
        comision_pago_usd=comision_pago_usd,
    )

    costo_pagina = costo_real.total_real_usd / vida if vida > 0 else 0.0
    costo_trabajo = costo_pagina * paginas

    return CostoCabezal(
        cantidad_cabezales=round(cantidad, 4),
        costo_base_cabezales_usd=round(costo_base_total, 4),
        impuesto_usd=round(costo_real.impuesto_usd, 4),
        delivery_usd=round(costo_real.delivery_usd, 4),
        comision_pago_usd=round(costo_real.comision_pago_usd, 4),
        costo_total_cabezales_usd=round(costo_real.total_real_usd, 4),
        costo_unitario_real_cabezal_usd=round(costo_real.costo_unitario_real_usd, 6),
        vida_util_paginas=round(vida, 4),
        costo_cabezal_por_pagina_usd=round(costo_pagina, 6),
        costo_cabezal_por_trabajo_usd=round(costo_trabajo, 6),
    )
